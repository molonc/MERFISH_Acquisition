disp('Andor SDK3 Data Conversion Example');
[rc] = AT_InitialiseLibrary();
AT_CheckError(rc);
[rc,hndl] = AT_Open(0);
AT_CheckError(rc);

disp('Camera initialized');
[rc] = AT_SetFloat(hndl,'ExposureTime',0.01);
AT_CheckWarning(rc);
[rc] = AT_SetInt(hndl,'FrameCount',1);
AT_CheckWarning(rc);
[rc,height] = AT_GetInt(hndl,'AOIHeight');
AT_CheckWarning(rc);
[rc,width] = AT_GetInt(hndl,'AOIWidth');
AT_CheckWarning(rc);

%Mono 12 Packed Acqusition
[rc] = AT_SetEnumIndex(hndl,'SimplePreAmpGainControl', 0);
AT_CheckWarning(rc);
[rc] = AT_SetEnumString(hndl,'PixelEncoding','Mono12Packed');
AT_CheckWarning(rc);
[rc,imagesize] = AT_GetInt(hndl,'ImageSizeBytes');
AT_CheckWarning(rc);
[rc] = AT_QueueBuffer(hndl,imagesize);
AT_CheckWarning(rc);
disp('Starting acquisition...');
[rc] = AT_Command(hndl,'AcquisitionStart');
AT_CheckWarning(rc);
[rc,buf] = AT_WaitBuffer(hndl,1000);
AT_CheckWarning(rc);
[rc,stride] = AT_GetInt(hndl,'AOIStride'); 
AT_CheckWarning(rc);
[rc,buf2] = AT_ConvertMono12PackedToMatrix(buf,height,width,stride);
AT_CheckWarning(rc);
h=imagesc(buf2);
drawnow;
[rc] = AT_Command(hndl,'AcquisitionStop');
AT_CheckWarning(rc);
disp('Acquisition complete');

%Mono 16 Acquisition (Mono 12 behaves the exact same way)
[rc] = AT_SetEnumString(hndl,'SimplePreAmpGainControl', '16-bit (low noise & high well capacity)');
AT_CheckWarning(rc);
[rc] = AT_SetEnumString(hndl,'PixelEncoding','Mono16');
AT_CheckWarning(rc);
[rc,imagesize] = AT_GetInt(hndl,'ImageSizeBytes');
AT_CheckWarning(rc);
[rc] = AT_QueueBuffer(hndl,imagesize);
AT_CheckWarning(rc);
disp('Starting acquisition...');
[rc] = AT_Command(hndl,'AcquisitionStart');
AT_CheckWarning(rc);
[rc,buf] = AT_WaitBuffer(hndl,1000);
AT_CheckWarning(rc);
[rc,stride] = AT_GetInt(hndl,'AOIStride'); 
AT_CheckWarning(rc);
[rc,buf2] = AT_ConvertMono16ToMatrix(buf,height,width,stride);
AT_CheckWarning(rc);
set(h,'CData',buf2);
drawnow;
[rc] = AT_Command(hndl,'AcquisitionStop');
AT_CheckWarning(rc);
disp('Acquisition complete');

%Mono 32 Acqusiition
 %Setting Accumulations, the most common use of Mono 32
[rc] = AT_SetInt(hndl,'AccumulateCount',5);
AT_CheckWarning(rc);
[rc] = AT_SetInt(hndl,'FrameCount',5);
AT_CheckWarning(rc);
[rc] = AT_SetEnumString(hndl,'PixelEncoding','Mono32');
AT_CheckWarning(rc);
[rc,imagesize] = AT_GetInt(hndl,'ImageSizeBytes');
AT_CheckWarning(rc);
[rc] = AT_QueueBuffer(hndl,imagesize);
AT_CheckWarning(rc);
disp('Starting acquisition...');
[rc] = AT_Command(hndl,'AcquisitionStart');
AT_CheckWarning(rc);
[rc,buf] = AT_WaitBuffer(hndl,1000);
AT_CheckWarning(rc);
[rc,stride] = AT_GetInt(hndl,'AOIStride'); 
AT_CheckWarning(rc);
[rc,buf2] = AT_ConvertMono32ToMatrix(buf,height,width,stride);
AT_CheckWarning(rc);
set(h,'CData',buf2);
drawnow;
[rc] = AT_Command(hndl,'AcquisitionStop');
AT_CheckWarning(rc);
disp('Acquisition complete');

[rc] = AT_Flush(hndl);
AT_CheckWarning(rc);
[rc] = AT_Close(hndl);
AT_CheckWarning(rc);
[rc] = AT_FinaliseLibrary();
AT_CheckWarning(rc);
disp('Camera shutdown');